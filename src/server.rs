...
use scrap::camera;
// `terminal-only`: use the no-op camera stub so `libs/scrap` is never compiled in.
#[cfg(feature = "terminal-only")]
use no_service::camera;
#[cfg(not(any(target_os = "android", target_os = "ios")))]
...
#[cfg(feature = "terminal-only")]
mod no_service;
// `terminal-only`: swap the real video / display / audio services for no-op stubs.
#[cfg(feature = "terminal-only")]
pub use no_service::{audio_service, display_service, video_service};
...
// Portable service is a Windows elevated *capture* helper built on
// `scrap::Capturer` / `Display` / `PixelBuffer`; see `no_service::portable_service`.
#[cfg(all(windows, not(feature = "terminal-only")))]
pub mod portable_service;
#[cfg(all(windows, feature = "terminal-only"))]
pub use no_service::portable_service;
mod service;
#[cfg(not(feature = "terminal-only"))]
mod video_qos;
#[cfg(not(feature = "terminal-only"))]
...
pub fn new() -> ServerPtr {
    ...
    #[cfg(not(target_os = "ios"))]
    {
        ...
        // `terminal-only`: no clipboard sync.
        #[cfg(not(feature = "terminal-only"))]
