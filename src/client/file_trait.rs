...
pub trait FileManager: Interface {
    ...
    // `read_dir` returns a `sciter::Value` and builds its result with the Sciter UI's
    // `make_fd`, so it only exists when the Sciter frontend does. File transfer itself
    // does not go through it -- the flutter/CLI-flavour callers never touch this method.
    #[cfg(not(any(
        target_os = "android",
        target_os = "ios",
        feature = "flutter",
