...
#[cfg(all(not(feature = "flutter"), not(feature = "terminal-only")))]
...
fn get_keyboard_mode() -> String {
    // `CUR_SESSION` only exists while a Sciter frontend owns the current session; a
    // headless build has none, and these fall through to their defaults instead.
    #[cfg(all(not(feature = "flutter"), not(feature = "terminal-only")))]
