...
#[cfg(all(feature = "unix-file-copy-paste", not(feature = "terminal-only")))]
use crate::clipboard::try_empty_clipboard_files;
#[cfg(all(
    not(any(target_os = "android", target_os = "ios")),
    not(feature = "terminal-only")
))]
...
impl Connection {
    pub async fn start(
            ...
            tokio::select! {
                ...
                Some(data) = rx_from_cm.recv() => {
                    match data {
                        ...
                        #[cfg(all(target_os = "windows", not(feature = "terminal-only")))]
        ...
        if conn.authorized {
            ...
            // 精简版没有常驻 UI 去显示新密码，不打印出来下一轮就没人知道密码了。
            #[cfg(feature = "terminal-only")]
            println!(
                "[rdterm] 一次性密码已更新: {}",
    ...
    fn check_update_temporary_password(&self, temporary_password_success: bool) {
        ...
        #[cfg(feature = "terminal-only")]
        println!("[rdterm] 一次性密码已更新: {new_password}");
        log::warn!(
    ...
    async fn on_message(&mut self, msg: Message) -> bool {
        ...
        if let Some(message::Union::LoginRequest(lr)) = msg.union {
            ...
            // rdterm（terminal-only）：无密码模式。
            // 入网凭证是那个 12 小时轮换的随机 ID，所以到这里直接放行，
            // 不再校验一次性密码 / 固定密码 / 2FA / 点击确认。
            // 注意必须放在上面之后 —— `self.terminal` 是在前面的 union match 里设的。
            #[cfg(feature = "terminal-only")]
            {
                // 注意：这里绝不能先设 `self.authorized = true`！
                // `send_logon_response_and_keep_alive` 开头就是 `if self.authorized { return true }`，
                // 预置之后它会直接返回、一条消息都不发，客户端收不到 PeerInfo，
                // 表现是「服务端日志说已放行，客户端却一直连不上」。
                log::info!("rdterm: accept login without password from {}", lr.my_id);
            ...
            match msg.union {
                ...
                #[cfg(all(any(target_os = "windows", feature = "unix-file-copy-paste"), not(feature = "terminal-only")))]
    ...
    async fn update_options(&mut self, o: &OptionMessage) {
        ...
        #[cfg(all(not(any(target_os = "android", target_os = "ios")), not(feature = "terminal-only")))]
...
/// 精简版的空实现：没有 UI 就没有 connection manager 进程，也就没有需要桥接的 IPC。
///
/// 真实的 `start_ipc` 在找不到 CM 时会去 spawn `SelfExe --cm`；命令行解析不认识
/// `--cm`，子进程会退化成「被控端」，跟本进程抢同一个 ID，把正在进行的会话踢掉。
#[cfg(all(
    feature = "terminal-only",
    not(any(target_os = "android", target_os = "ios"))
))]
async fn start_ipc(
    _rx_to_cm: mpsc::UnboundedReceiver<ipc::Data>,
    _tx_from_cm: mpsc::UnboundedSender<ipc::Data>,
) -> ResultType<()> {
    Ok(())
}
