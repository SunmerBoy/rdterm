...
#[cfg(windows)]
mod win_input;
// `win_mag` is the one privacy-mode backend that captures the screen
// (`scrap::CapturerMag`), so it cannot exist in a screen-capture-free build.
...
lazy_static::lazy_static! {
    static ref PRIVACY_MODE_CREATOR: Arc<Mutex<HashMap<&'static str, PrivacyModeCreator>>> = {
        ...
        #[cfg(windows)]
        {
            let exclude_supported = win_exclude_from_capture::is_supported();
            ...
            // Magnifier-based privacy mode needs screen capture; available everywhere else.
            #[cfg(all(windows, not(feature = "terminal-only")))]
            if !exclude_supported {
