// With `stub` on, `main()` returns immediately: the vcpkg / bindgen machinery at the
// bottom of this file is deliberately unreachable, so silence the dead-code noise it
// would otherwise generate rather than cfg-ing out five helpers and their imports.
#![cfg_attr(feature = "stub", allow(dead_code, unused_imports, unused_variables))]
...
fn main() {
    // `stub`: this crate is compiling the capture-free stand-in, so there is no
    // backend to configure. Skipping out here is the entire reason the stand-in
    // exists -- the calls below shell out to vcpkg for libvpx / aom / libyuv, and
    // a headless remote-terminal build must not need vcpkg at all.
    if env::var_os("CARGO_FEATURE_STUB").is_some() {
        // Keep the backend cfg flags defined so `cargo::rustc-check-cfg` stays honest.
