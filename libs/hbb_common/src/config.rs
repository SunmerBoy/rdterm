...
impl Config {
    ...
    pub fn path<P: AsRef<Path>>(p: P) -> PathBuf {
        // 绿色模式：APP_DIR 一旦被设置，所有配置都落在它下面（桌面端同样生效）。
        // rdterm 在启动早期把 APP_DIR 设为 exe 所在目录，从而不触碰 %APPDATA%。
        {
...
impl LanPeers {
    pub fn load() -> LanPeers {
        let _lock = CONFIG.read().unwrap();
        match fs::read_to_string(Config::file_("_lan_peers")) {
            Ok(s) => match toml::from_str(&s) {
                Ok(peers) => peers,
                Err(err) => {
